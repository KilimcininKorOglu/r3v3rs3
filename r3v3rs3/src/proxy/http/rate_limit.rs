...
use std::{
    ...
    net::IpAddr,
    num::NonZeroU32,
    sync::{Arc, Mutex, Once, PoisonError, Weak},
    time::Duration,
};
use tracing::debug;
...
static REGISTRY: Lazy<Mutex<HashMap<LimiterKey, Weak<ClientRateLimiter>>>> =
    Lazy::new(Default::default);
static CLEANUP_TASK: Once = Once::new();
...
impl ClientRateLimiter {
    fn new(config: RateLimit) -> Option<Self> {
        let quota = Quota::with_period(config.replenish_interval()?)?
...
/// Returns the limiter for `key`, or `None` when the limit is disabled. The existing limiter is
/// reused while its configuration is unchanged, so a configuration reload keeps the client counters.
pub fn limiter(key: LimiterKey, config: RateLimit) -> Option<Arc<ClientRateLimiter>> {
    start_cleanup_task();
    let mut registry = REGISTRY.lock().unwrap_or_else(PoisonError::into_inner);
    registry.retain(|_, limiter| limiter.strong_count() > 0);
    if let Some(existing) = registry.get(&key).and_then(Weak::upgrade) {
        if existing.config == config {
            return Some(existing);
        }
    }
    let limiter = Arc::new(ClientRateLimiter::new(config)?);
    registry.insert(key, Arc::downgrade(&limiter));
    Some(limiter)
}
...
mod tests {
    ...
    fn disabled_limit_has_no_limiter() {
        assert!(limiter(("rldis".parse().unwrap(), None), RateLimit::default()).is_none());
    }

    #[test]
    fn limiter_rejects_clients_over_the_burst() {
        let limiter = limiter(("rlburst".parse().unwrap(), None), per_minute(2)).unwrap();
    ...
    fn unchanged_config_reuses_the_limiter() {
        ...
        let first = limiter(key, per_minute(5)).unwrap();
        let second = limiter(key, per_minute(5)).unwrap();
        assert!(Arc::ptr_eq(&first, &second));

        let changed = limiter(key, per_minute(6)).unwrap();
