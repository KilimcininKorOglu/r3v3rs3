...
Platform geliştirme aşamasındadır. Bu sürüm, registry'deki hazır bir image'ı yönetim API'si üzerinden yerel Docker Engine'e deploy eder. WebUI sayfaları, Git ve Dockerfile build'leri ve uzak sunucular sonraki sürümlerde gelir.
...
1. r3v3rs3 image'ı çeker ve digest'ini kaydeder.
...
`DELETE /api/apps/{id}` uygulamanın container'larını ve network'ünü durdurup siler, proxy'sini kaldırır, environment değişkenlerini ve deployment'larını siler. Adlandırılmış volume'lar kalır.
